"""Audit confederate session lists: trial-type counts and label problems."""
...
REWARDED = frozenset({"RA1", "RA2", "RA3", "RA4"})
...
def audit_labels(
    ...
    *,
    ...
    for trial_type in TRIAL_TYPES:
        value, type_problems = _count_rewarded_for_type(labels, trial_type, list_monkey)
    ...
    return SessionAuditRow(
        session_id=session_id_prefix(session_id),
        dataset=dataset,
        problem="-" if not unique_problems else "; ".join(unique_problems),
...
def _row_fieldnames() -> list[str]:
    return ["session_id", "dataset", "problem", *COUNT_COLUMNS]
