...
from analyze_stability.plots_lr import configure_array_time_axis, trial_trace_sd
import matplotlib.pyplot as plt
import numpy as np
from scipy.io import loadmat
from scipy.stats import wilcoxon

from process_channels.features import run_mann_whitney
...
def collect_pooled_trials(
    ...
    *,
    ...
    """Pool smoothed L/R trials across sessions using the requested processing mode."""
    ...
    for session_id in session_ids:
        if layout == "curated":
            session_dir = data_root / condition_key / session_id
        else:
            session_dir = data_root / session_id
        event = alignment_event_for_recording(session_id, recording_monkey)
        ...
        try:
            ...
            zscore_ref = (
                zscore_reference_mask(labels, recording_monkey, session_id=session_id)
                if zscore_mua
                else None
            )
        ...
        session_t_ms = load_time_vector(event_dir, session_id, event, pre_post_tag)
        ...
        for ch_num, ch_path in channel_files_by_number(event_dir).items():
            try:
    ...
    if t_ms is None or win_idx is None:
        raise ValueError(f"No usable sessions (n={len(session_ids)})")
...
def collect_pooled_data(
    ...
    filters = trial_filters or trial_filters_for_condition(condition, actor_side)
    return collect_pooled_trials(
        session_ids,
        data_root=Path(DATA_ROOT),
...
def plot_from_pooled(
    ...
    """Plot combined L/R PDFs from pre-pooled trial data."""
    trial_filters = trial_filters or TRIAL_FILTERS
    if trial_filters is None:
        monkey = recording_monkey_from_condition_label(condition)
        actor_side = recording_actor_side(session_ids[0], monkey)
        trial_filters = trial_filters_for_condition(condition, actor_side)
    print(f"\n=== {condition} | {len(session_ids)} session(s) -> {output_dir} ===")

    n_left_total = sum(t.shape[0] for parts in pooled.left_by_ch.values() for t in parts)
    n_right_total = sum(t.shape[0] for parts in pooled.right_by_ch.values() for t in parts)
    ...
    if ctx is not None:
        alignment_label = ctx.alignment_event_label(session_ids)
    else:
        events = {
            alignment_event_for_recording(
                sid, recording_monkey(session_id=sid, condition_label=condition),
            )
            for sid in session_ids
        }
        alignment_label = events.pop() if len(events) == 1 else "actor-side fixation release"
    suptitle_base = (
        f"{condition} | combined sessions | {alignment_label}\n"
    ...
    for array_index, array_name in enumerate(ARRAY_NAMES):
        fig = make_array_figure(
            array_index, pooled.t_ms, pooled.win_idx,
            pooled.left_by_ch, pooled.right_by_ch,
            pooled.sess_left_by_ch, pooled.sess_right_by_ch,
            f"{suptitle_base} | {array_name}",
        )
        out_path = output_dir / f"{condition}_{alignment_label}_{array_name}_combined_LR.pdf"
...
def plot_condition_combined(
    ...
    monkey = recording_monkey_from_condition_label(condition)
    actor_side = recording_actor_side(session_ids[0], monkey)
    trial_filters = TRIAL_FILTERS or trial_filters_for_condition(condition, actor_side)
