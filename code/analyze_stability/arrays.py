...
from load_data.io import ARRAY_NAMES, array_nominal_channels, filter_summary
...
from analyze_stability.plots_lr import configure_array_time_axis
...
# Array-mean panels: all trials pooled per channel (vstack), then mean±SE across channels.
ARRAY_MEAN_AGGREGATION_LABEL = (
    "all-trial pool per ch, then mean ± SE across channels"
)
...
def plot_condition_array_combined(
    ctx: PipelineContext,
    pooled: "PooledTrialData | None" = None,
) -> None:
    from load_data.cache import require_pooled_disk_cache
    ...
    if pooled is None:
        pooled = require_pooled_disk_cache(ctx.output_base, ctx.condition_label, True)
    ...
    suptitle = (
        f"{ctx.condition_label} | {ARRAY_MEAN_AGGREGATION_LABEL} | {alignment_label}\n"
        f"{filter_summary(ctx.trial_filters)} | "
        f"L/R {ctx.choice_field} {ctx.left_choice}/{ctx.right_choice} | "
        f"{proc_label} | n_sessions={len(session_ids)}"
    )

    fig = make_arrays_combined_figure(
        pooled.t_ms, pooled.win_idx, pooled.left_by_ch, pooled.right_by_ch, suptitle,
    )
    out_path = output_dir / f"{ctx.condition_label}_{alignment_label}_arrays_combined_LR.pdf"
