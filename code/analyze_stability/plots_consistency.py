...
from analyze_stability.metrics import ChannelStability
    ...
    *,
    ...
    for idx, sid in enumerate(session_ids):
        ...
        if s is None:
            ...
            ax.text(
                0.5, 0.5, "missing", transform=ax.transAxes,
    ...
    if t_ms is not None:
        time_panel = min((nrows - 1) * ncols, len(session_ids) - 1)
        configure_array_time_axis(axes_flat, t_ms, time_panel=time_panel)

    if ylim is not None:
        for row in range(nrows):
            col0 = row * ncols
            if col0 >= len(session_ids):
                break
            axes_flat[col0].set_ylabel(y_label, fontsize=7)
            axes_flat[col0].tick_params(axis="y", labelleft=True)

    fig.suptitle(title, fontsize=10)
    bottom = 0.10 if nrows > 1 else 0.14
    fig.subplots_adjust(left=0.08, right=0.98, top=0.88, bottom=bottom, hspace=0.35, wspace=0.25)
    fig.savefig(out_path, format="pdf", dpi=150, bbox_inches="tight")
    plt.close(fig)
